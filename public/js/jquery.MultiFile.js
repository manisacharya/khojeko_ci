...
if (window.jQuery)(function ($) {
			...
			.each(function () {
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						var v = String(file.name || '' ).replace(/[&<>'"]/g, function(c) { return '&#'+c.charCodeAt()+';'; }),
								...
								label =	$(
										(
											...
											+'<br>'+(MultiFile.preview || $(slave).is('.with-preview') ? p : '' )+
						...
						names[names.length] =
							(
								'<span class="MultiFile-title" title="' + MultiFile.STRING.file + '">'
									+ MultiFile.STRING.selected +'<br>'+'<a id="primary">Set as Primary</a>'+
								'</span>'
							)
							.replace(/\$(file|name)/gi, (v.match(/[^\/\\]+$/gi)||[v])[0])
							.replace(/\$(ext|extension|type)/gi, (v.match(/[^\.]+$/gi)||[''])[0])
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		// previews
		preview: false,
		previewCss: 'height:100px;width:100px;border-radius:5px;',
