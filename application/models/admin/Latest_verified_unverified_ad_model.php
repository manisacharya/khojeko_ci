...
class Latest_verified_unverified_ad_model extends CI_Model {
    ...
    public function get_details_item_personal($id){
        ...
        $this->db->join('user', "(items.user_id = user.user_id AND user.type='personal' AND items.is_verified=$id AND items.deleted_date=0)");
        $this->db->join('personal', "user.user_key = personal.p_id ");
        $this->db->join('item_img', "(items.item_id = item_img.item_id AND item_img.primary=1)");
    ...
    public function get_details_item_dealer($id){
        ...
        $this->db->select('*')->from('items');
        $this->db->join('user', "(items.user_id = user.user_id AND user.type='dealer' AND items.is_verified=$id AND items.deleted_date=0)");
